...
public:
    Dijkstra(DigrafoValorado<int> const& g, int orig) : origen(orig), dist(g.V(), INF), ulti(g.V()), pq(g.V()) {
        dist[origen] = 0;
        pq.push(origen, 0);
        while (!pq.empty()) {
    ...
    deque<int> camino(int v) const {        
        // recuperamos el camino avanzando (es un cola doble podemos mirar ambos lados)
        deque<int> cam;
        AristaDirigida<int> a;
        // Empezamos desde el destino y retrocedemos
        cam.push_front(v + 1); // Destino

        int actual = v;

        while (actual != origen) {
            a = ulti[actual];
            cam.push_front(a.desde() + 1);
            actual = a.desde();
        }
...
bool resuelveCaso() {
    ...
    for (int i = 0; i < K; i++) {
        ...
        Dijkstra repartos(dgv, v1 - 1);

        if (repartos.hayCamino(v2 - 1)) {
            cout << repartos.distancia(v2 - 1) << ": ";
            deque<int> camino = repartos.camino(v2 - 1);
            while (!camino.empty()) {
                cout << camino.front();
                camino.pop_front();
                if (!camino.empty()) {
