...
protected:
   ...
   void rotaDer(Link & r2) {
      Link r1 = r2->iz;
      r2->iz = r1->dr;
      r2->tam_i -= r1->tam_i; //r2 pierde el tamaño de r1
      r1->dr = r2;
   ...
   void rotaIzq(Link & r1) {
      Link r2 = r1->dr;
      r1->dr = r2->iz;
      r2->tam_i += r1->tam_i; //r2 gana el tamaño de r1
      r2->iz = r1;
      r1->altura = std::max(altura(r1->iz), altura(r1->dr)) + 1;
      r2->altura = std::max(altura(r2->iz), altura(r2->dr)) + 1;
      r1 = r2;
   ...
   T const& kesimo_rec(Link a, int k) const {
       if (a == nullptr) {
           throw std::out_of_range("Posicion no encontrada");
       }

       if (a->tam_i < k) {
           // buscamos en el subarbol derecho
           if (a->dr == nullptr) throw std::out_of_range("??");
           return kesimo_rec(a->dr, k - a->tam_i);
       }
       else if (a->tam_i == k) {
           // devolvemos el valor si lo encontramos en este nodo
           return a->elem;
       }
       else {
           // buscamos en el subarbol izquierdo
           if (a->iz == nullptr) throw std::out_of_range("??");
           return kesimo_rec(a->iz, k);
       }
   }
